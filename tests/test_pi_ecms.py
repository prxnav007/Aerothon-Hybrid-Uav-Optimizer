...
def test_ratio_anchor_is_recovered_exactly_at_the_reference_state_of_charge() -> None:
    ...
    ctx = _context(soc=controller.soc_ref, switching_s=6.0)
...
def test_ratio_anchor_tracks_the_operating_point_without_scaling_feedback() -> None:
    ...
    low = controller.equivalence_factor(_context(soc=0.3, switching_s=4.0))
    high = controller.equivalence_factor(_context(soc=0.3, switching_s=7.0))
    assert high - low == pytest.approx(1.2 * (7.0 - 4.0), rel=1.0e-12)


def test_absolute_anchor_ignores_the_operating_point_references() -> None:
    controller = PIECMS(s0=6.5, s0_ratio=None)
    low = controller.equivalence_factor(
        _context(soc=0.3, neutral_s=4.0, switching_s=3.0)
    )
    high = controller.equivalence_factor(
        _context(soc=0.3, neutral_s=9.0, switching_s=8.0)
    )
    assert high == low
...
def test_default_reachable_range_straddles_the_switching_factor() -> None:
    controller = PIECMS()
...
def test_absolute_anchor_diagnostics_use_the_same_raw_feedback_law() -> None:
    ...
    assert controller.straddles_switching(7.0)
...
def test_high_anchor_and_low_gain_can_fail_to_straddle_without_raising() -> None:
    ...
    assert not controller.straddles_switching(6.0)
...
def test_diagnostics_reject_a_non_physical_switching_factor(
    ...
    with pytest.raises(ValueError, match="switching_s"):
        PIECMS().reachable_range(switching_s)
